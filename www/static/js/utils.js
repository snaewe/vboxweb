/*
 * $Id: utils.js 57 2010-08-20 13:44:33Z imooreyahoo $
...
function vboxAjaxRequest(fn,params,callback,xtra) {
	...
	return jQuery.post('/ajax?fn='+fn, params,
		function(d) {
				...
				if(d.errors.length > 0) {
					...
					for(var i = 0; i < d.errors.length; i++) {
						...
						if($('#vboxIndex').data('vboxConfig') &&
								$('#vboxIndex').data('vboxConfig').VBWEB_ERRNO_FATAL &&
								d.errors[i].errno == $('#vboxIndex').data('vboxConfig').VBWEB_ERRNO_FATAL) {
...
function vboxAjaxError(e) {
	...
	div.innerHTML = '<img src="images/50px-Warning_icon.svg.png" style="float: left; padding: 10px;" /><p>'+$('<div />').html(e.error).text()+'</p>';
    ...
    $(div).dialog({'closeOnEscape':false,'width':400,'height':'auto','buttons':buttons,'modal':true,'autoOpen':true,'stack':true,'dialogClass':'vboxDialogContent','title':'VirtualBox Web Console'});			
...
function vboxAlert(msg,xtraOpts) {
	...
	var dialogOpts = {'closeOnEscape':false,'width':'50%','height':'auto','buttons':buttons,'modal':true,'autoOpen':true,'stack':true,'dialogClass':'vboxDialogContent','title':'VirtualBox Web Console'};
...
function vboxConfirm(q,buttons) {
    ...
    $(div).dialog({'closeOnEscape':false,'width':500,'height':'auto','buttons':buttons,'modal':true,'autoOpen':true,'stack':true,'dialogClass':'vboxDialogContent','title':'VirtualBox Web Console'});			
...
function vboxProgress(pid,callback,args) {
	...
	div.setAttribute('title','VirtualBox Web Console');
...
function trans(w) {
	...
	if(!$('#vboxIndex').data('vboxLangData')) alert(w);
	
	if(__vboxLangContext && $('#vboxIndex').data('vboxLangData')[__vboxLangContext+'::'+w])
		return $('#vboxIndex').data('vboxLangData')[__vboxLangContext+'::'+w];
	
	return ($('#vboxIndex').data('vboxLangData')[w] ? $('#vboxIndex').data('vboxLangData')[w] : w);
	// debug -
	//return ($('#vboxIndex').data('vboxLangData')[w] ? $('#vboxIndex').data('vboxLangData')[w] : alert("'" + w + "' not defined in language file."));
}
